...
<body>
  ...
  <?php
	...
	$sql = "SELECT planet_name, mercenary, mercenary_time, defender, declared FROM `match` WHERE match_id=" . $match_id . " AND NOT(ISNULL(mercenary));";
	...
	if ($row = mysqli_fetch_row($result)) {
		if ($row[2] == "") {
			$result_html = "You currently have a contract offer to " . $row[1] . " to attack " . $row[3] . " on planet " . $row[0] .
				"<br>Offer was sent on " . $row[4];
		} else {
			$result_html = "Contract was accepted by " . $row[1] . " to fight " . $row[3] . " on planet " . $row[0] .
